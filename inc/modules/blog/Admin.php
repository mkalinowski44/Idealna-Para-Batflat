...
class Admin extends AdminModule
{
    ...
    public function navigation()
    {
        return [
            ...
            $this->lang('tags')                 => 'manageTags',
            $this->lang('settings')             => 'settings',
            $this->lang('manage_comments')      => 'comments'
        ];
    }
    ...
    public function anyManage($page = 1)
    {
            ...
            if (isset($_POST['post-list']) && !empty($_POST['post-list'])) {
                foreach ($_POST['post-list'] as $item) {
                    ...
                    if ($this->db('blog')->delete($item) === 1) {
                        ...
                        $this->db('blog_tags_relationship')->delete('blog_id', $item);
        ...
        if ($totalRecords) {
            ...
            foreach ($rows as $row) {
                ...
                $row['published_timestamp'] = $row['published_at'];
    ...
    public function getManageTags($page = 1)
    {
        // lang
        if (!empty($_GET['lang'])) {
            $lang = $_GET['lang'];
        ...
        // pagination
        $countQuery = $this->db()->pdo()->prepare('SELECT COUNT(*) AS count FROM (SELECT blog_tags.id,blog_tags.name,blog_tags.slug,COUNT(blog_tags_relationship.tag_id) AS count FROM blog_tags LEFT JOIN blog_tags_relationship ON blog_tags.id = blog_tags_relationship.tag_id GROUP BY blog_tags.id ORDER BY count DESC)');
        $countQuery->execute();
        $totalRecords = $countQuery->fetchAll();
        $totalRecords = $totalRecords[0]['count'];
        $pagination = new \Inc\Core\Lib\Pagination($page, $totalRecords, 10, url([ADMIN, 'blog', 'manageTags', '%d']));
        $assign['pagination'] = $pagination->nav();

        $assign['tagsCount'] = $totalRecords;

        $rows = $this->db('blog_tags')
            ->leftJoin('blog_tags_relationship', 'blog_tags.id = blog_tags_relationship.tag_id')
            ->select(['blog_tags.id', 'blog_tags.name', 'blog_tags.slug', 'count' => 'COUNT(blog_tags_relationship.tag_id)'])
            ->group('blog_tags.id')
            ->desc('count')
            ->limit($pagination->offset() . ', ' . $pagination->getRecordsPerPage())
            ->toArray();
        ...
        foreach ($rows as &$row) {
            $row['delURL'] = url([ADMIN, 'blog', 'deleteTag', $row['id'], $page]);
        }

        $assign['items'] = $rows;
        $assign['updateURL'] = url([ADMIN, 'blog', 'updateTags', $page]);
        $assign['addURL'] = url([ADMIN, 'blog', 'addTag', $page]);

        return $this->draw('manage_tags.html', ['tags' => $assign]);
    }

    public function postAddTag($page)
    {
        if (!isset($_POST['name'])) {
            $this->notify('failure', $this->lang('save_failure'));
            redirect(url([ADMIN, 'blog', 'manageTags', $page]));
        }

        $tag = $_POST['name'];
        if ($this->db('blog_tags')->where('name', $tag)->oneArray()) {
            $this->notify('failure', $this->lang('tag_already_exists'));
            redirect(url([ADMIN, 'blog', 'manageTags', $page]));
        }
        ...
        while ($this->db('blog_tags')->where('name', '!=', $tag)->where('slug', $slug)->oneArray()) {
            $slug = $oldSlug . '-' . ($i);
            $i++;
        }

        if ($this->db('blog_tags')->save(['name' => $tag, 'slug' => $slug])) {
    ...
    public function getDeleteTag($id, $page)
    {
        $error = false;
        try {
            $this->db('blog_tags')->delete($id);
            $this->db('blog_tags_relationship')->delete('tag_id', $id);
        } catch (Exception $e) {
            $error = true;
        }
        if ($error) {
    ...
    public function postUpdateTags($page)
    {
        if (!isset($_POST['edit-list'])) {
            redirect(url([ADMIN, 'blog', 'manageTags', $page]));
        }

        $tags = $_POST['edit-list'];
        $query = true;

        foreach ($tags as $id => $tag) {
            if ($searchedTag = $this->db('blog_tags')->where('name', $tag)->oneArray()) {
                if ($searchedTag['id'] == $id) {
                    continue;
                } else {
                    $this->notify('failure', $tag . ': ' . $this->lang('tag_already_exists'));
                    redirect(url([ADMIN, 'blog', 'manageTags', $page]));
                }
            }

            if (preg_match("/[`~!@#$%^&*()_|+\-=?;:\'\",.<>\{\}\[\]\\\/]+/", $tag)) {
                $this->notify('failure', $tag . ': ' . $this->lang('save_tags_failure'));
                redirect(url([ADMIN, 'blog', 'manageTags', $page]));
            }
            ...
            while ($this->db('blog_tags')->where('name', '!=', $tag)->where('slug', $slug)->where('id', '!=', $id)->oneArray()) {
                $slug = $oldSlug . '-' . ($i);
                $i++;
            }
            $query = $this->db('blog_tags')->where('id', $id)->save(['name' => $tag, 'slug' => $slug]);
        }

        if ($query) {
            $this->notify('success', $this->lang('save_tags_success'));
        } else {
            $this->notify('failure', $this->lang('save_tags_failure'));
        }
    ...
    public function postSave($id = null)
    {
        ...
        if (isset($_FILES['cover_photo']['tmp_name'])) {
            ...
            $thumbnail = new \Inc\Core\Lib\Image;
            $mobile = new \Inc\Core\Lib\Image;

            if ($img->load($_FILES['cover_photo']['tmp_name'])) {
                $img->resize(1920);
                $img->crop(0, ($img->getInfos('height') / 2) - 200, 1920, 400);

                $_POST['cover_photo'] = $_POST['slug'] . "." . $img->getInfos('type');
            }

            if ($thumbnail->load($_FILES['cover_photo']['tmp_name'])) {
                $thumbnail->resize(760);
                $thumbnail->crop(0, ($thumbnail->getInfos('height') / 2) - 70, 760, 140);

                $_POST['cover_thumbnail'] = $_POST['slug'] . "-thumbnail." . $thumbnail->getInfos('type');
            }

            if ($mobile->load($_FILES['cover_photo']['tmp_name'])) {
                $mobile->resize(760);
                $mobile->crop(($mobile->getInfos('width') / 2) - 380, ($mobile->getInfos('height') / 2) - 200, 760, 400);

                $_POST['cover_mobile'] = $_POST['slug'] . "-mobile." . $mobile->getInfos('type');
            }
        }
        ...
        if ($query) {
            ...
            if ($img->getInfos('width')) {
                $img->save(UPLOADS . "/blog/" . $_POST['cover_photo']);
            }

            if ($thumbnail->getInfos('width')) {
                $thumbnail->save(UPLOADS . "/blog/" . $_POST['cover_thumbnail']);
            }
            if ($mobile->getInfos('width')) {
                $mobile->save(UPLOADS . "/blog/" . $_POST['cover_mobile']);
            }
    ...
    public function getDelete($id)
    {
        if ($post = $this->db('blog')->where('id', $id)->oneArray() && $this->db('blog')->delete($id)) {
            ...
            if ($post['cover_thumbnail']) {
                unlink(UPLOADS . "/blog/" . $post['cover_thumbnail']);
            }
            if ($post['cover_mobile']) {
                unlink(UPLOADS . "/blog/" . $post['cover_mobile']);
            }
            $this->notify('success', $this->lang('delete_success'));
        } else {
            $this->notify('failure', $this->lang('delete_failure'));
    ...
    public function getDeleteCover($id)
    {
        if ($post = $this->db('blog')->where('id', $id)->oneArray()) {
            ...
            $this->db('blog')->where('id', $id)->save(['cover_photo' => null, 'cover_thumbnail' => null, 'cover_mobile' => null]);
    ...
    {
        $disqus = $this->settings('blog.disqus');
        return $this->draw('comments.html', ['disqus' => $disqus]);
    }
