...
class Admin extends AdminModule
{
    ...
    public function getEdit($id, $page = 1)
    {
        ...
        if (count($rows)) {
            foreach ($rows as $row) {
                ...
                if($assign['settings']['sort'] == 'ASC') {
                    $row['upURL'] = url([ADMIN, 'galleries', 'changeOrder', 'up', $row['id'], $row['gallery'], $assign['page']]);
                    $row['downURL'] = url([ADMIN, 'galleries', 'changeOrder', 'down', $row['id'], $row['gallery'], $assign['page']]);
                } else {
                    $row['upURL'] = url([ADMIN, 'galleries', 'changeOrder', 'down', $row['id'], $row['gallery'], $assign['page']]);
                    $row['downURL'] = url([ADMIN, 'galleries', 'changeOrder', 'up', $row['id'], $row['gallery'], $assign['page']]);
                }
    ...
    * change order
    */
    public function getChangeOrder($direction, $id, $gallery, $page)
    {
        $item = $this->db('galleries_items')->oneArray($id);

        if (!empty($item)) {
            if ($direction == 'up') {
                $nextItem = $this->db('galleries_items')
            ...
            if (!empty($nextItem)) {
                $this->db('galleries_items')->where('id', $item['id'])->save(['"order"' => $nextItem['order']]);
                $this->db('galleries_items')->where('id', $nextItem['id'])->save(['"order"' => $item['order']]);
            }
        }
        redirect(url(ADMIN.'/galleries/edit/' . $gallery . "/" . $page));
    }
        ...
        if (!is_uploaded_file($_FILES['files']['tmp_name'][0])) {
            ...
            foreach ($_FILES['files']['tmp_name'] as $image) {
                ...
                if ($img->load($image)) {
                    ...
                    $order = $this->db('galleries_items')->where('gallery', $id)->count();
                    $query = $this->db('galleries_items')->save(['src' => serialize($src), 'gallery' => $id, '"order"' => $order]);
                } else {
