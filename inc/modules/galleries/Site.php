    ...
    private function _importGalleries()
    {
        ...
        if (count($galleries)) {
            foreach ($galleries as $gallery) {
                if ($gallery['sort'] == 'ASC') {
                    $items = $this->db('galleries_items')->where('gallery', $gallery['id'])->asc('`order`')->toArray();
                } else {
                    $items = $this->db('galleries_items')->where('gallery', $gallery['id'])->desc('`order`')->toArray();
                }
                ...
                if (count($items)) {
                    foreach ($items as &$item) {
                        ...
                        if (!isset($item['src']['md'])) {
                            $item['src']['md'] = $item['src']['sm'];
                        }
                    }
